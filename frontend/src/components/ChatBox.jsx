...
import { sendMessage, clearChat, checkNetworkStatus } from "../api";
...
export default function ChatBox() {
  ...
  const [isOnline, setIsOnline] = useState(false); // Auto-detected, not manual
  ...
  useEffect(() => {
    // 1. Browser-level instant detection
    const handleOnline = () => {
      console.log('Browser detected: ONLINE');
      checkNetworkStatus().then(status => {
        setIsOnline(status.cerebras_available);
        if (status.cerebras_available) {
          addNotification('success', 'Online Mode', 'Connected to Cerebras AI');
        }
      });
    };
    ...
    const handleOffline = () => {
      ...
      setIsOnline(false);
      addNotification('info', 'Offline Mode', 'Using local model');
    };
    ...
    checkNetworkStatus().then(status => {
      setIsOnline(status.cerebras_available);
      console.log(`Initial network status: ${status.cerebras_available ? 'ONLINE' : 'OFFLINE'}`);
    });
    ...
    const interval = setInterval(async () => {
      ...
      const nowOnline = status.cerebras_available;
      
      if (wasOnline !== nowOnline) {
        console.log(`Network status changed: ${nowOnline ? 'ONLINE' : 'OFFLINE'}`);
        if (nowOnline) {
          addNotification('success', 'Back Online', 'Switched to Cerebras AI');
        } else {
          addNotification('info', 'Connection Lost', 'Switched to local model');
        }
      }
    ...
    <div className={`flex h-screen ${isDarkTheme ? 'bg-gray-900 text-white' : 'bg-white text-gray-900'}`}>
      ...
      <div className={`${sidebarOpen ? 'w-64' : 'w-16'} transition-all duration-300 ease-in-out ${
        ...
        <div className={`${sidebarOpen ? 'p-3 space-y-3' : 'p-2 space-y-2'}`}>
          {/* Network Status Indicator (Auto-detected, not clickable) */}
          <div
            className={`w-full flex items-center ${sidebarOpen ? 'gap-3 px-4 py-3 justify-between' : 'justify-center p-3'} rounded-xl transition-all duration-300 ${
              isOnline 
                ? isDarkTheme
                  ? "bg-gradient-to-r from-green-900 to-emerald-900 text-green-100 shadow-lg shadow-green-900/50 border border-green-700/50" 
                  : "bg-gradient-to-r from-green-50 to-emerald-50 text-green-800 shadow-md shadow-green-200/50 border border-green-200"
                : isDarkTheme
                  ? "bg-gradient-to-r from-gray-700 to-gray-800 text-gray-300 shadow-lg shadow-gray-900/50 border border-gray-600/50"
                  : "bg-gradient-to-r from-gray-50 to-slate-50 text-gray-700 shadow-md shadow-gray-200/50 border border-gray-200"
            }`}
            title={!sidebarOpen ? (isOnline ? "Online - Using Cerebras" : "Offline - Using Local Model") : ""}
          >
            <span className="flex items-center gap-2.5">
              <span className={`text-xl flex-shrink-0 ${isOnline ? 'animate-pulse' : ''}`}>
                {isOnline ? "🌐" : "📱"}
              </span>
              {sidebarOpen && (
